...
Jumper2left::Jumper2left(int x, int y) : Enemy(x, y)
{
	...
	jump.PushBack({ 683, 208, 16, 48 });//en vuelo
	//jump.PushBack({ 700, 208, 16, 48 });//crouch
	...
	path.PushBack({ -0.1f, 1.2f }, 78, &jump);
	path.PushBack({ 0.0f, 0.0f }, 55, &shotidle);
	path.PushBack({ -1.0f, 0.0f }, 110, &left);
...
void Jumper2left::Move()
{
	...
	if (path.GetFrame() == 418 || isdead == true && dead.Finished() == true) {
		to_delete = true;
	}

}
void Jumper2left::OnCollision(Collider* c1, Collider* c2)
{
	path.Erase();
