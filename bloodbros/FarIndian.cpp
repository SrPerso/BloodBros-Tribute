...
FarIndian::FarIndian(int x, int y) : Enemy(x, y)
{
	...
	original_pos.x = x;
	original_pos.y = y;
	path.PushBack({ -0.6f, 0.0f }, 110, &fly);
	...
	path.PushBack({ -0.6f, 0.0f }, 40, &fly);
	path.PushBack({ 0.0f, 0.0f }, 30, &idle);
	path.PushBack({- 0.6f, 0.0f }, 180, &fly);
...
void FarIndian::OnCollision(Collider* c1, Collider* c2)
{
	path.Erase();
