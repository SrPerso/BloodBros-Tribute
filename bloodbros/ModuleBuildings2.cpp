...
update_status ModuleBuilding2::Update()
{

	for (uint i = 0; i < MAX_BUILDINGS; ++i)
	{
		Building2* p = active[i];
		...
		else if ((p->hits <= 1 && p->mytype != WINDMILL2 || (p->mytype == WINDMILL2 && p->hits <= 2)) || (p->mytype == TREES2 && p->hits == 0) || (p->mytype == ROCK2 && p->hits <= 2))
		{
			if (p->mytype == WINDMILL2){
		...
		else if (p->mytype == TREES2 && p->hits >= 1){
		...
		else if (p->hits > 1 && p->mytype != WINDMILL2 && p->mytype != TREES2 && p->mytype != ROCK2){
		...
		else if (p->hits > 2 && p->mytype == WINDMILL2){
			...
			if (p->destroy.Finished()){
				if (p->mytype == WINDMILL2){ windmillalive = false; }
				delete p;
				active[i] = nullptr;
			}
		}
		else if (p->hits > 2 && p->mytype == ROCK2){
			delete p;
			active[i] = nullptr;
		}
	}
...
void ModuleBuilding2::AddBuilding2(const Building2& particle, int x, int y)
{
	Building2* p = new Building2(particle);
	p->position.x = x;
	p->position.y = y;
	if (p->mytype == PURPLE2){ purplealive = true; }
	if (p->mytype == YELLOW2){ yellowalive = true; }
	if (p->mytype == WINDMILL2){ windmillalive = true; }
	if (p->mytype == WHEEL2){
		p->collider = App->collision->AddCollider({ p->position.x, p->position.y, particle.build.w, particle.build.h }, COLLIDER_NONE, this);
	}
	else if (p->mytype == TREES2){
...
const Collider* Building2::get_collider() const
{
	return collider;
}
...
Building2::Building2(const Building2& p) :
...
bool Building2::Update()
{
	bool ret = true;

	return ret;
}
void ModuleBuilding2::OnCollision(Collider* c1, Collider* c2)
{
	if (App->player->hit == true){
		App->player->hit = false;
		for (uint i = 0; i < MAX_BUILDINGS; ++i){
			if (active[i] != nullptr && active[i]->get_collider() == c1 && active[i]->mytype == TREES2){
				active[i]->hits += 1;
				break;
			}
			if (active[i] != nullptr && active[i]->get_collider() == c1 && active[i]->mytype == YELLOW2){
				if (active[i]->hits == 0){
			...
			if (active[i] != nullptr && active[i]->get_collider() == c1 && active[i]->mytype == PURPLE2){
				if (active[i]->hits == 0){
			...
			if (active[i] != nullptr && active[i]->get_collider() == c1 && active[i]->mytype == WINDMILL2){
				if (active[i]->hits == 0){
					active[i]->build.x = 53;
			...
			if (active[i] != nullptr && active[i]->get_collider() == c1 && active[i]->mytype == ROCK2){
				if (active[i]->hits == 0){
					active[i]->build.x = 66;
